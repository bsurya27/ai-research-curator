...
def search_arxiv(
    ...
    Args:
        ...
        category: If set, restrict to this arXiv category via ``cat:`` in the query
            ...
            filter is applied.
        sort_by: How arXiv sorts results before the date filter: ``submitted_date``
            (default), ``relevance``, or ``last_updated_date``. Unknown values fall
            back to submitted date with a warning.
